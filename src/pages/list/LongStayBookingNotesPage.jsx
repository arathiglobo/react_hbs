/**
 * LongStayBookingNotesPage.jsx
 *
 * Dedicated notes page for a single Long Stay booking. Mirrors the hotel
 * BookingNotesPage but wires to the Long Stay notes endpoints:
 *   - GET  /api/longStayBooking/{id}/notes
 *           → array of { id, longStayBookingId, note, createdBy, createdDate }
 *   - POST /api/longStayBooking/{id}/notes  body { note, createdBy? }
 *
 * Note the field names differ from the hotel flow: the long-stay note
 * body field is `note` (not `noteText`) and the timestamp is `createdDate`
 * (not `createdAt`), and the GET returns a bare array rather than a
 * { success, notes } envelope.
 */
...
export default function LongStayBookingNotesPage() {
  const { id } = useParams();
