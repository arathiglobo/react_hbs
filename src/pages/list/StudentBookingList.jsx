 ...
 *
 * Booking-list page for the student flow. UI mirrors HotelBookingList.jsx
 * exactly:
 ...
 *
 * The student flow no longer has an admin verification / approval step, so
 * BookingType + Time-Period filtering is applied client-side over the rows
 * returned by GET /api/student-booking/list (which returns every field used
 * below, including deadlineDate + paymentMode).
 */
...
const getPaymentStatusLabel = (displayStatus) => {
  ...
  // cancellation, so check this ahead of everything else — compositeStatus puts
  ...
  // On Request chain (see compositeStatus), so it falls through to "Paid".
  ...
  const fetchPage = async () => {
    ...
    try {
      ...
      const { data } = await axiosInstance.get(
        `/api/student-booking/list?${params.toString()}`
      );
          ...
          <Container fluid className="px-0">
            ...
            <div className="d-flex justify-content-between align-items-end mb-3 hbl-header">
              <div className="hbl-header-left">
                <h3 className="fw-bold text-dark mb-2">Student Bookings</h3>
                            ...
                            return (
                              ...
                              >
                                ...
                                {/* Payment Status — derived from the same
                                    ...
                                    renders: Confirmed / On Request → Payment
                                    Pending, ReConfirmed → Paid, a cancellation →
                                    Paid or Un-Paid depending on whether it had
                                    been reconfirmed. See getPaymentStatusLabel. */}
                                <td
                                  style={{
                                    ...baseCellStyle,
                                    textAlign: "center",
                                ...
                                >
                                  <div className="d-flex justify-content-center align-items-center">
                                    <FaEye
                                      ...
                                      onClick={() =>
                                        navigate(`/booking-details/student-booking/${b.bookingId}`)
                                      }
                                      onKeyDown={(e) => {
                                        if (e.key === "Enter" || e.key === " ") {
                                          e.preventDefault();
