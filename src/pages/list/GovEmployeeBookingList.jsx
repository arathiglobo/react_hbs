                              ...
                              return (
                                <tr key={r.bookingId}>
                                  ...
                                  <td className="text-end" style={{ whiteSpace: "nowrap" }}>
                                    {(() => {
                                      // Show the booked currency when present
                                      // (displayAmount = totalRate × factor);
                                      // derive the factor to convert the
                                      // strike-through pre-discount too.
                                      const conv =
                                        r.displayCurrencyCode &&
                                        r.displayCurrencyCode !== "AED" &&
                                        Number(r.displayAmount) > 0 &&
                                        Number(r.totalRate) > 0
                                          ? Number(r.displayAmount) / Number(r.totalRate)
                                          : 1;
                                      const code = conv !== 1 ? r.displayCurrencyCode : "AED";
                                      return (
                                        <>
                                          <div className="fw-semibold text-dark">
                                            {r.totalRate != null
                                              ? `${code} ${(Number(r.totalRate) * conv).toFixed(2)}`
                                              : "-"}
                                          </div>
                                          {r.totalRateBeforeDiscount != null &&
                                            r.totalRateBeforeDiscount !== r.totalRate && (
                                              ...
                                              >
                                                {code}{" "}
                                                {(Number(r.totalRateBeforeDiscount) * conv).toFixed(2)}
                                              </div>
                                            )}
                                        </>
                                      );
