/**
 * CabBookingNotesPage.jsx
 *
 * Dedicated notes page for a single Cab booking. Mirrors
 * LongStayBookingNotesPage but wires to the cab notes endpoints:
 *   - GET  /api/cab/{id}/notes
 *           → array of { id, cabBookingId, note, createdBy, createdDate }
 *   - POST /api/cab/{id}/notes  body { note, createdBy? }
 *
 * `id` here is the package booking id (CustomBookPackage.custombookingId)
 * the cab detail view keys rows by — the same id every other cab action
 * endpoint uses. The note body field is `note` and the timestamp is
 * `createdDate`; the GET returns a bare array.
 */
...
export default function CabBookingNotesPage() {
  const { id } = useParams();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const createdBy =
        ...
        "unknown";
      const res = await axiosInstance.post(`/api/cab/${id}/notes`, {
        note: noteText.trim(),
        createdBy,
      });
