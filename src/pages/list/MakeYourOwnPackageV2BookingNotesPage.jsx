/**
 * MakeYourOwnPackageV2BookingNotesPage.jsx
 *
 * Dedicated notes page for a single Make-Your-Own-Package v2 booking.
 * Mirrors LongStayBookingNotesPage but wires to the v2 notes endpoints:
 *   - GET  /api/makeYourOwnPackageV2/booking/{id}/notes
 *           → array of { id, bookingId, note, createdBy, createdDate }
 *   - POST /api/makeYourOwnPackageV2/booking/{id}/notes  body { note, createdBy? }
 *
 * The note body field is `note` and the timestamp is `createdDate`; the
 * GET returns a bare array rather than a { success, notes } envelope.
 */
...
export default function MakeYourOwnPackageV2BookingNotesPage() {
  const { id } = useParams();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await axiosInstance.post(
        `/api/makeYourOwnPackageV2/booking/${id}/notes`,
