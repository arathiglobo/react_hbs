...
export default function GovEmployeeBookingDetailView() {
  ...
  const _gvDispCode = data?.displayCurrencyCode;
  const _gvAedTotal = Number(data?.totalRate) || 0;
  const _gvDispAmt = Number(data?.displayAmount);
  const _gvConv =
    _gvDispCode &&
    _gvDispCode !== "AED" &&
    Number.isFinite(_gvDispAmt) &&
    _gvDispAmt > 0 &&
    _gvAedTotal > 0
      ? _gvDispAmt / _gvAedTotal
      : 1;
  const curCode = _gvConv !== 1 ? _gvDispCode : "AED";
  const dconv = (aed) => (Number(aed) || 0) * _gvConv;
                    ...
                    <Row>
                      ...
                      <Col md={6}>
                        ...
                        {/* Optional "Booking Done By Employee" — only
                            rendered when picked at search time. */}
                        {data.employeeName && (
                          <InfoRow
