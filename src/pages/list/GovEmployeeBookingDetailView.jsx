 ...
 *
 * Detail view for a gov-employee booking. UI + functionality EXACTLY mirror
 * StudentBookingDetailView.jsx — same cards, section order, two-status flow,
 * action buttons and modals — only the endpoints (/api/gov-employee-booking),
 * routes, and the verification card (Government Employee instead of Student)
 * differ.
 *
 * Action set (all backed by /api/gov-employee-booking):
 *   - ADD NEW ITEM        → /new-booking/gov-employee?parentBookingCode=<code>
 *   - CANCEL              → DELETE /api/gov-employee-booking/:id
 *   - RECONFIRM           → PATCH  /api/gov-employee-booking/:id/reconfirm
 *   - VOUCHER / INVOICE / PROFORMA → GET /api/gov-employee-booking/:id/document?type=
 *   - ADD AGENT REFERENCE → GET/POST /api/gov-employee-booking/:id/agent-reference
 *   - CONFIRMATION NO.    → POST   /api/gov-employee-booking/:id/confirmation-no
 *   - BOOKING REMARK      → POST   /api/gov-employee-booking/:id/remark
 *   - RESEND MAIL TO AGENT→ POST   /api/gov-employee-booking/:id/resend-mail
 *   - NOTES               → GET/POST /api/gov-employee-booking/:id/notes
 */
...
// Source of the pg-option-* classes used by the Select Payment Gateway
// modal below — same import BookingDetailedView.jsx uses so the
// gov-employee detail picker renders identically to the hotel detail
// picker.
...
// BookingDetailedView + the gov-employee booking page.
...
// Online-payment gateways — mirrors HotelBookingPage / BookingDetailedView.
// CCAvenue is the only real gateway wired; the dummy /payment/:gateway
// path is kept as a fallback for any future non-CCAvenue entry.
...
export default function GovEmployeeBookingDetailView() {
  ...
  // — agents still see the RECONFIRM button. Mirrors the identical gate on
  // BookingDetailedView / LongStayBookingDetailView / DayStayBookingDetailView.
  ...
  // once on mount and sent on the cancel/reject DELETE so the BE can stamp
  ...
  // GovEmployeeBookingController fills it in from the HTTP request itself.
  // Same two-step resolution as the booking page: coarse IP-derived city
  ...
  // deferred-credit (Voucher-Later) or On-Request booking and the agent
  ...
  // Resend mail — clicking the button opens a preview modal that shows the
  // Voucher PDF in an iframe and lets the admin confirm / edit the agent's
  // email before the /resend-mail POST fires.
  ...
  // for gov-employee bookings since the backend does not surface an
  ...
  const fetchBooking = useCallback(() => {
    ...
    return axiosInstance
      .get(`/api/gov-employee-booking/${id}`)
      .then((res) => {
  ...
  const fetchNotes = useCallback(() => {
    if (!id) return;
    axiosInstance
      .get(`/api/gov-employee-booking/${id}/notes`)
      .then((res) => {
  ...
  // status the booking held BEFORE cancellation. Gov-employee backend may
  ...
  const statusColor = (() => {
    ...
    if (normalizedStatus === "CONFIRMED") return "#198754"; // confirmed → green
  ...
  // Reject — only reachable for On-Request bookings. Gov-employee backend
  // has no dedicated reject endpoint, so this calls the cancel path with
  // a "rejected by …" reason so the lifecycle progresses and the audit
  // trail captures the rejector. Mirrors BookingDetailedView semantically:
  // a rejected on-request becomes Cancelled.
  const rejectBooking = async () => {
    ...
    try {
      ...
      const res = await axiosInstance.delete(`/api/gov-employee-booking/${id}`, {
  ...
  const runReconfirm = async () => {
    try {
      ...
      const res = await axiosInstance.patch(
        `/api/gov-employee-booking/${id}/reconfirm`,
        {
  ...
  //   • Deferred-credit bookings (creditDeferred === true — Voucher-Later
  //     creates whose debit the BE deferred to this Reconfirm step; gov
  //     can't reuse the hotel's voucherGenerated marker because it stamps
  //     "On Reconfirmation" on every booking).
  ...
  // CC Avenue post-payment resume — mirrors BookingDetailedView /
  // DayStayBookingDetailView. CC Avenue's redirect is a real
  // cross-domain browser navigation, so React Router `state`
  // (resumeReconfirm above) never survives it; the backend instead
  // appends ?ccavenueOrderId=&ccavenueStatus= to the URL when it
  // redirects back here. Unlike the dummy-gateway path, no runReconfirm
  // call is made — the backend replays the reconfirm itself using the
  // transaction row persisted at /initiate time, with
  // alreadyPaidOnline=true so credit isn't touched and
  // paymentMode="ONLINE" is stamped.
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const response = await axiosInstance.post(
          `/api/payment/ccavenue/finalize-govemployee-reconfirm/${ccavenueOrderId}`,
        );
        ...
        console.error("Post-payment gov-employee reconfirm finalize failed:", err);
        toast.error(
  ...
  const openAgentRefModal = async () => {
    ...
    try {
      const res = await axiosInstance.get(`/api/gov-employee-booking/${id}/agent-reference`);
  ...
  const saveAgentRef = async () => {
    ...
    try {
      ...
      const res = await axiosInstance.post(`/api/gov-employee-booking/${id}/agent-reference`, {
        agentLpo: v,
      });
      if (res.data?.success) {
  ...
  const saveConfirmationNo = async () => {
    ...
    try {
      ...
      const res = await axiosInstance.post(`/api/gov-employee-booking/${id}/confirmation-no`, {
  ...
  const saveNote = async () => {
    ...
    try {
      ...
      const res = await axiosInstance.post(
        `/api/gov-employee-booking/${id}/notes`,
  ...
  const saveRemark = async () => {
    ...
    try {
      ...
      const res = await axiosInstance.post(`/api/gov-employee-booking/${id}/remark`, { remarks: text });
      if (res.data?.success !== false) {
  ...
  // Proforma Invoice via GET /api/gov-employee-booking/:id/document?type=...
  ...
  const handleDocument = async (type, label) => {
    try {
      ...
      const res = await axiosInstance.get(`/api/gov-employee-booking/${id}/document`, {
        params: { type },
      });
   ...
   *   1) /document?type=VOUCHER — the same PDF the mail will attach,
  ...
  const submitResendMail = async () => {
    ...
    try {
      ...
      const res = await axiosInstance.post(
        `/api/gov-employee-booking/${id}/resend-mail`,
        null,
        { params: { email } },
      );
      if (res.data?.success) {
  ...
  // mirrors /api/booking-document-email/{id}; if the gov-employee backend
  // hasn't wired it yet, the modal surfaces the failure clearly.
  ...
  const sendDocumentEmail = async () => {
    ...
    try {
      ...
      const res = await axiosInstance.post(
        `/api/gov-employee-booking-document-email/${id}`,
        {
          email,
  ...
  const bookingHistory = (() => {
    ...
    // treatment on BookingDetailedView / LongStayBookingDetailView /
    // DayStayBookingDetailView.
  ...
  return (
    <div className="min-vh-100 bg-light d-flex flex-column">
                    ...
                    <div style={{ padding: "12px 16px" }}>
                      <Row>
                        <Col md={6}>
                          ...
                          <InfoRow label="Nationality" value={c.nativeCountry || c.customerNationality} />
                        </Col>
                        <Col md={6}>
                          <InfoRow label="Passport No." value={c.passportNo} />
                          <InfoRow label="Agent LPO" value={c.agentLpo} />
                        </Col>
                      </Row>
                    </div>
                  </div>
                    ...
                    <Row>
                      <Col md={6}>
                        <InfoRow
                          ...
                          value={
                            booking.verificationMethod === "GOVT_ID_UPLOAD"
                        ...
                        />
                        <InfoRow label="Employee Code" value={booking.govEmployeeCode} />
                        <InfoRow label="Employee Name" value={booking.govEmployeeName} />
                      </Col>
                      <Col md={6}>
                            ...
                            value={
                              booking.govtIdFilePath ? (
                                <a
                                  ...
                                  rel="noreferrer"
                                >
                                  {booking.govtIdFileName || "View uploaded ID"}
                                </a>
                              ) : (
                                "-"
                              )
      ...
      {/* ── Select Payment Gateway ──
          Identical to the hotel detail picker (BookingDetailedView) and
          the day-stay detail picker.
          On Proceed:
            • CC Avenue → real billing-page redirect via the shared
              CCAvenueCheckoutPage; the browser fully navigates away to
              CC Avenue's hosted page and back with
              ?ccavenueOrderId=&ccavenueStatus= appended, which the
              resume effect below catches and finalizes.
            • Any other gateway → dummy /payment/:gateway page with
              location.state.resumeReconfirm = true so the older
              resume effect fires runReconfirm directly. */}
      <Modal
            ...
            {PAYMENT_GATEWAYS.map((g) => {
              ...
              return (
                ...
                >
                  <input
                    type="radio"
                    name="gov-reconfirm-payment-gateway"
                    id={`gov-reconfirm-gw-${g.id}`}
        ...
        <Modal.Footer className="border-0">
          ...
          <Button
            ...
            onClick={() => {
              ...
              // resume effect below. reconfirmBookingId tells
              // CCAvenueCheckoutPage to pay off THIS existing booking.
              // flowType is explicit (unlike hotel, which defaults to
              // RECONFIRM when only reconfirmBookingId is set) so
              // /initiate reaches the GOVEMP_RECONFIRM arm.
              if (selectedGateway === "ccavenue") {
                ...
                navigate("/payment/ccavenue-redirect", {
                  state: {
                    ...
                    flowType: "GOVEMP_RECONFIRM",
                  },
                });
                return;
              }

              // Dummy /payment/:gateway flow (test/local only) — kept
              // for parity with BookingDetailedView; no gateway
              // currently routes through it because CCAvenue is the
              // only PAYMENT_GATEWAYS entry.
      ...
      {/* PDF Preview Modal — renders Voucher / Proforma Voucher / Invoice /
          ...
          /api/gov-employee-booking/:id/document. Mirrors the pattern used
          ...
          {pdfPreview?.url && (
            <>
              ...
              <Button
                ...
                as="a"
                href={pdfPreview.url}
                download={`GovEmployee_${id}_${pdfPreview.type || "document"}.pdf`}
              >
                Download
              </Button>
        ...
        <Modal.Header closeButton>
          <Modal.Title style={{ fontSize: "1rem", display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span>
              Booking History
              {booking?.bookingCode && (
                <span style={{ opacity: 0.85, fontWeight: 500 }}>{` — ${booking.bookingCode}`}</span>
              )}
            </span>
          </Modal.Title>
        </Modal.Header>
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Status column — the resulting booking state right
                            ...
                            Mirrors the identical treatment on
                            BookingDetailedView / LongStayBookingDetailView /
                            DayStayBookingDetailView. */}
                        <td style={{ padding: "10px 14px", borderBottom: "1px solid #eef2f6" }}>
                          {(() => {
      ...
      {/* ── Resend Mail to Agent — preview + send ────────────────────────
          ...
          button POSTs /api/gov-employee-booking/:id/resend-mail?email=…
