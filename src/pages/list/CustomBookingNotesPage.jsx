/**
 * CustomBookingNotesPage.jsx
 *
 * Dedicated notes page for a single Custom (Make Your Own Package) booking.
 * Mirrors LongStayBookingNotesPage but wires to the custom-booking notes
 * endpoints:
 *   - GET  /api/makeYourOwnPackage/{id}/notes
 *           → array of { id, customBookingId, note, createdBy, createdDate }
 *   - POST /api/makeYourOwnPackage/{id}/notes  body { note, createdBy? }
 *
 * The GET returns a bare array; the note body field is `note` and the
 * timestamp is `createdDate`.
 */
...
export default function CustomBookingNotesPage() {
  const { id } = useParams();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await axiosInstance.post(
        `/api/makeYourOwnPackage/${id}/notes`,
