...
const AgentsAutoGeneratedInvoice = () => {
  ...
  const fetchInvoiceHistory = async (pageNum = 0, searchTerm = search) => {
    setIsLoading(true);
    try {
      console.log("Fetching invoice history for agentId:", id); // Debug log
  ...
  const getInvoiceStatusBadge = (status) => {
    ...
    const badgeClass = badgeClasses[statusType] || 'bg-secondary';
    ...
    <div className="min-vh-100 bg-light d-flex flex-column">
      ...
      <div className="d-flex flex-grow-1">
        ...
        <main className="flex-grow-1 p-4">
          ...
          {/* Title */}
          <div className="mb-4">
            <h2 className="fw-bold mb-2">Auto Generated Invoice</h2>
          ...
          <Card className="shadow-sm rounded-xl">
            <Card.Header className="d-flex justify-content-between align-items-center">
              <span className="fw-semibold">Invoice History</span>
              {/* Search Bar */}
                  ...
                  {items.map((item, index) => (
                    <tr key={item.id || index}>
                      ...
                      <td className="fw-bold">
                        {item.invoiceNumber || `INV-${item.id || index + 1}`}
                      </td>
                      <td className="fw-bold text-primary">
                        ${formatAmount(item.amount)}
                      </td>
                      <td>{getInvoiceStatusBadge(item.status)}</td>
                ...
                <div className="d-flex justify-content-between align-items-center p-3 border-top">
                  <div>
                    <small className="text-muted">
                      Showing {items.length} of {totalPages * 10} invoices
                    </small>
                  </div>
                  <div>
                    <Pagination className="mb-0">
                      <Pagination.Prev
                        disabled={page === 0}
                        onClick={() => fetchInvoiceHistory(page - 1, search)}
                      />
                      {Array.from({ length: totalPages }, (_, i) => (
                        <Pagination.Item
                          key={i}
                          active={i === page}
                          onClick={() => fetchInvoiceHistory(i, search)}
                        >
                          {i + 1}
                        </Pagination.Item>
                      ))}
