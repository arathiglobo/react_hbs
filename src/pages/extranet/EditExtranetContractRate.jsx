...
export default function EditExtranetContractRate() {
  const navigate = useNavigate();
  const { id, editId } = useParams(); // id = hotelId, editId = contract rate ID
  ...
  useEffect(() => {
    const fetchContractRateData = async () => {
      if (!editId || markets.length === 0 || countries.length === 0) return;
      try {
        setFetchingData(true);
        const res = await axiosInstance.get(`/api/hotelContractRate/${editId}`);
    ...
    try {
      ...
      const payload = {
        ...
        contractRateValidityDTO: formData.validityList.map((v) => ({
          contractValidityId: "",
          validityFrom: v.validityFrom && v.validityFrom.length === 16 
            ? `${v.validityFrom}:00` 
            : v.validityFrom,
          validityTo: v.validityTo && v.validityTo.length === 16 
            ? `${v.validityTo}:00` 
            : v.validityTo,
        })),
      ...
      if (res.status === 200 || res.status === 201) {
        ...
        navigate(`/extranet/${id}/contract-rate`);
      }
    } catch (err) {
              ...
              {loading ? (
                ...
                <>
                  ...
                  <div className="d-flex justify-content-between mt-4">
                    <Button
                      variant="outline-danger"
                      onClick={() => navigate(`/extranet/${id}/contract-rate`)}
                    >
                      Cancel
                    </Button>
