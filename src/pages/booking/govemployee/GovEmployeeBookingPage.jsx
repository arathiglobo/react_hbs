...
// CCAvenue is the only real gateway wired; a dummy /payment/:gateway path
// is kept as a fallback for any future non-CCAvenue entry.
...
// SeniorCitizenBookingPage / StudentBookingPage.
...
const GovEmployeeBookingPage = () => {
  ...
  // DayStayBookingPage. Pairs with the beforeunload guard.
  ...
  // from the create request itself. Mirrors SeniorCitizenBookingPage /
  // StudentBookingPage.
  ...
  useEffect(() => {
    ...
    // same orderId returns the already-created booking.
    const finalizeAfterCCAvenue = async () => {
      try {
        setIsFinalizingPayment(true);
        setIsSubmitting(true);
        const { data } = await axiosInstance.post(
          `/api/payment/ccavenue/finalize-govemployee/${ccavenueOrderId}`,
        );
        if (data?.success) {
          toast.success(
            data.message ||
        ...
        const beMsg =
          err?.response?.data?.message || err?.response?.data?.error || null;
        console.error("Post-payment gov-employee finalize failed:", err);
        toast.error(
          beMsg ||
            "Payment succeeded but booking could not be created. Please contact support with your payment reference.",
        );
  ...
  const resolvedConfirmationStatus = (() => {
    // On Request rooms land on the tentative "Confirmed" step (mirrors the
    // hotel flow, where the engine stamps On Request creates CONFIRMED) so
    // the detail view can run the two-step CONFIRM → RECONFIRM flow. They
    // must never resolve to ReConfirmed at create.
    if (isOnRequestRate) return "Confirmed";
  ...
  const handleSubmit = (e) => {
    ...
    const built = {
      ...
      isOutsideDeadline,
      // Availability of the selected rate ("Available" / "On Request").
      // The BE payment-gate + two-step Confirm flow key off this: without
      // it the create endpoint can't recognise On Request bookings — it
      // would check-and-reject them on low credit ("Insufficient agent
      // credit limit…") and the detail view's CONFIRM/RECONFIRM split
      // would never activate.
      roomStatus: bookingData?.selectedRate?.roomStatus || "Available",
      source: "WEB",
  ...
  const confirmBooking = async () => {
    ...
    try {
      ...
      //     BE now skips their create-time debit too).
      ...
      try {
        const agentIdNum = Number(String(pendingPayload.agentId || "").trim());
        const requiredAmount = (pendingPayload.rooms || []).reduce(
          (s, r) => s + Number(r.rate || 0),
          0,
        );
        if (
          Number.isFinite(agentIdNum) &&
          agentIdNum > 0 &&
          requiredAmount > 0
        ) {
          ...
          if (creditResponse.data === false) {
            ...
            console.log(
              isOnRequestRate
                ? "[GOV_EMPLOYEE] Insufficient credit but On Request rate — proceeding with create."
                : "[GOV_EMPLOYEE] Insufficient credit but Voucher Later selected — proceeding with create; credit deducted at Reconfirm.",
            );
          }
        }
      } catch (err) {
  ...
  return (
    <div className="min-vh-100 bg-light d-flex flex-column hotel-booking-container">
          ...
          <Container fluid="xl">
            ...
            <Form onSubmit={(e) => e.preventDefault()}>
              ...
              {/* ─── Insufficient credit + card disabled → block booking ───
                  ...
                  shape as HotelBookingPage / DayStayBookingPage. */}
              <Modal
              ...
              {/* ─── Select Payment Gateway ───
                  ...
                  HotelBookingPage / DayStayBookingPage. The card renders
                  the CC Avenue logo from /public/ccavanue.png. */}
              <Modal
                ...
                <Modal.Footer className="border-0">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      // List labels the row correctly (mirrors the
                      ...
                      // and /finalize-govemployee replays it after payment.
                      // flowType tells the shared CCAvenueCheckoutPage to
                      // ask the backend for the GOVEMP_CREATE arm.
                      ...
                      // kept for parity with the other booking pages; no
      ...
      {/* ── Post-payment finalize overlay ──
          ...
          Mirrors HotelBookingPage / DayStayBookingPage. */}
      {isFinalizingPayment && (
        <div
          role="alertdialog"
          aria-modal="true"
